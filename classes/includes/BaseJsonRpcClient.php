...
class BaseJsonRpcClient
{
    ...
    protected function processCalls($calls)
    {
        ...
        $result = $this->batchCalls ? array_values(array_map('\mod_jirbis\includes\BaseJsonRpcCall::GetCallData', $calls)) :
        ...
        try {
            ...
            if ($this->login) {
                $options += array(CURLOPT_HTTPAUTH => CURLAUTH_BASIC) +
                    array(CURLOPT_USERPWD => $this->login . ":" . $this->password);
            }
